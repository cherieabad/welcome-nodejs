...
!function( $ ) {
	...
	var Datepicker = function(element, options) {
		...
		//Replace '-' with '_' in input language
		if (options.language) {
			var inputLang = options.language;
			options.language = inputLang.replace('-','_');
		}
		this.language = options.language||this.element.data('date-language')||"en";
		this.language = this.language in dates ? this.language : this.language.split('_')[0]; //Check if "de_DE" style date is available, if not language should fallback to 2 letter code eg "de"
		...
		formatDate: function(date, format, language){
			...
			for (var i=0, cnt = format.parts.length; i < cnt; i++) {
				...
				date.push(val[format.parts[i]]);
                        ...
                        $('#isoFormatYYYYMMDD').val(val.yyyy + val.mm + val.dd);
	...
	DPGlobal.template = '<div class="datepicker">'+
							...
							'<a class="button datepicker-close small alert right" style="width:auto;"><i class="fa fa-remove fa-times fi-x"></i></a>'+
                                                        '<input type="hidden" id="isoFormatYYYYMMDD" name="isoFormatYYYYMMDD"/>'+
						'</div>';
